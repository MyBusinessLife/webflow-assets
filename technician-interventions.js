(() => {
  if (window.__techInterventionsLoaded || window.__techInterventionsBooting) return;
  window.__techInterventionsBooting = true;

  (async () => {
    const root = await waitForRoot(12000);
    if (!root) {
      console.error("❌ Root introuvable");
      window.__techInterventionsBooting = false;
      return;
    }

    const supabaseReady = await waitForSupabase(12000);
    ...
    function renderProducts(container, interventionId) {
      ...
      const totalWrap = container.closest(".ti-block");
      const totalEl = totalWrap ? totalWrap.querySelector("[data-products-total]") : null;
      if (totalEl) totalEl.textContent = `Total: ${formatMoney(total)} | A rembourser: ${formatMoney(totalPaidByTech)}`;
...
.ti-panel{background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:10px}
...
.ti-validate-actions{margin-top:12px}
...
@media (min-width:768px){.ti-controls{grid-template-columns:1fr 280px;align-items:center}}
    ...
    function waitForRoot(timeoutMs = 8000) {
      const t0 = Date.now();
