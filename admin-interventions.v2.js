...
window.Webflow.push(async function () {
  ...
  function goStep(idx) {
    ...
    if (modalState.mode === "view" && currentStep === STEPS.length - 1) nextBtn.textContent = "Fermer";
    else if (currentStep === STEPS.length - 1) nextBtn.textContent = "Enregistrer";
  ...
  function ensureModalExists() {
    ...
    modal.querySelector(".itv-next").addEventListener("click", () => {
      ...
      showError("");
      if (modalState.mode === "view" && currentStep === STEPS.length - 1) { closeModal(); return; }
  ...
  function renderCompRows(existing = null) {
    ...
    if (!selected.length && existing?.length) {
      selected = existing.map(c => ({ id: c.tech_id, name: techNameFromCache(c.tech_id) }));
    }
  ...
  function updateCompTotalView() {
    const modal = ensureModalExists();
    const el = modal.querySelector(".comp-total");
    if (el) el.textContent = formatCents(computeCompTotalCents());
  }
  ...
  function updateExpenseTotalView() {
    const modal = ensureModalExists();
    const el = modal.querySelector(".exp-total");
    if (el) el.textContent = formatCents(computeExpenseTotalCents());
  }
